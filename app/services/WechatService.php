...
class WechatService {
	...
	private static $links = array(
			...
			'media_download' => 'https://api.weixin.qq.com/cgi-bin/media/get',
			'media_upload' => 'https://api.weixin.qq.com/cgi-bin/media/upload',
			'media_add_news'=> 'https://api.weixin.qq.com/cgi-bin/material/add_news',
	...
	public function handleRequest() {
		...
		if (isset($_GET['echostr']))
		{
			...
			file_put_contents(LOG_DIR."/post.txt",$GLOBALS['HTTP_RAW_POST_DATA']);
	 ...
	 * 获取微信服务器IP地址
	 * 
	 * @return Array [127.0.0.1,127.0.0.1,127.0.0.1] 
	 */
	...
	public function requestText() {
		$text = array();
		$text["text"] = strval($this->postObj->Content);
		return $text;
	}
	 ...
	 * 获取事件的基本信息
	**/
	public function requestEventInfo() {
		$info = array();
		$info['Event'] = $this->postObj->EventKey;
		$info['Ticket'] = $this->postObj->Ticket;
		$info['Event'] = str_replace('qrscene_', '', $this->postObj->EventKey);
	...
	// 获取下载链接
	public function mediaDownloadUrl($mediaid){
		if ($this->getAccessToken()){
			return self::$links['media_download'] . "?access_token={$this->access_token}&media_id=" . $mediaid;
		}
		return "#";
	}
	 ...
	 * @return bool|mixed
	 */
	public function materialUpload($filetype, $filepath,$materialdesc = array()) {
		if ($this->getAccessToken() && file_exists($filepath))
		{
			$media = array();
			$fileext = strtolower(Tools::getFileExtension($filepath));
			...
			switch ($filetype)
			{
				...
				case self::MEDIA_TYPE_VIDEO:
					if ($fileext != 'mp4' || $filesize > 1024 * 1024){
						return false;
					}elseif(!isset($materialdesc["introduction"]) || !isset($materialdesc["title"])){
						return false;
					}
					break;
				case self::MEDIA_TYPE_THUMB:
			...
			$media['access_token'] = $this->access_token;
			$media['type'] = $filetype;
			$media['description'] = json_encode($materialdesc);
			$media['media'] = '@' . $filepath;

			return self::post(self::$links['upload_material'], $media);
		}
	...
	*/
	public function imageUpload($filepath){
		if ($this->getAccessToken() && file_exists($filepath))
		{
			...
			return self::post(self::$links['upload_image'], $media);
		}
